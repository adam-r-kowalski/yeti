...
fn sub_fn(comptime T: type) fn (T, T) T {
    return struct {
        fn f(x: T, y: T) T {
            return x - y;
        }
...
fn mul_fn(comptime T: type) fn (T, T) T {
    return struct {
        fn f(x: T, y: T) T {
            return x * y;
        }
...
fn div_fn(comptime T: type) fn (T, T) T {
    if (T == i64) {
        return struct {
            fn f(x: T, y: T) T {
                return @divExact(x, y);
...
fn eq_fn(comptime T: type) fn (T, T) i32 {
    return struct {
        fn f(x: T, y: T) i32 {
            return if (x == y) 1 else 0;
        }
...
fn or_fn(comptime T: type) fn (T, T) T {
    return struct {
        fn f(x: T, y: T) T {
            return x | y;
        }
...
const eq_binary_ops = ComparisonBinaryOps{
    .I64 = .i64_eq,
    .I32 = .i32_eq,
    .U64 = .i64_eq,
    .U32 = .i32_eq,
...
const bit_or_binary_ops = IntBinaryOps{
    .I64 = .i64_or,
    .I32 = .i32_or,
    .U64 = .i64_or,
    .U32 = .i32_or,
...
fn commonType(t1: Entity, t2: Entity) ?Entity {
    if (eql(t1, t2)) return t1;
    const b = t1.ecs.get(components.Builtins);
    const builtins = [_]Entity{ b.I64, b.I32, b.U64, b.U32, b.F64, b.F32 };
    const float_builtins = [_]Entity{ b.F64, b.F32 };
    if (eql(t1, b.IntLiteral)) {
        for (builtins) |builtin| {
            if (eql(t2, builtin)) return builtin;
        }
        return null;
    }
    if (eql(t2, b.IntLiteral)) {
        for (builtins) |builtin| {
            if (eql(t1, builtin)) return builtin;
        }
        return null;
    }
    if (eql(t1, b.FloatLiteral)) {
        for (float_builtins) |builtin| {
            if (eql(t2, builtin)) return builtin;
        }
        return null;
    }
    if (eql(t2, b.FloatLiteral)) {
        for (float_builtins) |builtin| {
            if (eql(t1, builtin)) return builtin;
        }
        return null;
...
fn unifyTypes(e1: Entity, e2: Entity) !Entity {
    const t1 = typeOf(e1);
    const t2 = typeOf(e2);
    if (commonType(t1, t2)) |common_type| {
        _ = try e1.set(.{components.Type.init(common_type)});
        _ = try e2.set(.{components.Type.init(common_type)});
        return common_type;
    }
    return error.CannotUnifyTypes;
}
...
fn Context(comptime FileSystem: type) type {
    return struct {
        ...
        fn lowerIf(self: *Self, entity: Entity) !Entity {
            ...
            if (eql(typeOf(conditional), builtins.IntLiteral)) {
                const value = try i64Of(conditional);
                if (value != 0) {
                    const then = entity.get(components.Then).slice();
                    var then_entity: Entity = undefined;
                    for (then) |expression| {
                        then_entity = try self.lowerExpression(expression);
                    }
                    return then_entity;
                } else {
                    ...
                    for (else_) |expression| {
                        else_entity = try self.lowerExpression(expression);
                    }
                    return else_entity;
                }
            }
            const instruction = try self.codebase.createEntity(.{
                components.IrInstructionKind.if_,
        ...
        fn lowerExpression(self: *Self, entity: Entity) error{ Overflow, InvalidCharacter, OutOfMemory, CantOpenFile, CannotUnifyTypes }!Entity {
...
test "lower int literal" {
    ...
    for (types) |type_, i| {
        ...
        try expectEqual(typeOf(five), builtin_types[i]);
    }
}
...
test "lower float literal" {
    ...
    for (types) |type_, i| {
        ...
        try expectEqual(typeOf(five_three), builtin_types[i]);
    }
}
...
test "call function from import" {
    ...
    for (types) |type_, i| {
        ...
        const baz = blk: {
            ...
            try expectEqual(typeOf(bar_baz), builtin_types[i]);
            try expectEqual(call.get(components.Arguments).len(), 0);
            break :blk call.get(components.Callable).entity;
        };
...
test "lower assignments with explicit float type" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
...
test "lower int literal binary op" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
    ...
    const builtin_types = [_]Entity{ builtins.I64, builtins.I32, builtins.U64, builtins.U32, builtins.F64, builtins.F32 };
    const op_strings = [_][]const u8{ "+", "-", "*", "/" };
    const answers = [_][]const u8{ "6", "2", "8", "2" };
    for (op_strings) |op_string, op_index| {
        for (types) |type_, i| {
            var fs = try MockFileSystem.init(&arena);
            _ = try fs.newFile("foo.yeti", try std.fmt.allocPrint(&arena.allocator,
            ...
            try expectEqual(basic_block.len, 3);
            {
                ...
                try expectEqual(int_const.get(components.IrInstructionKind), .int_const);
                const result = int_const.get(components.Result).entity;
                try expectEqualStrings(literalOf(result), "4");
                try expectEqual(typeOf(result), builtins.IntLiteral);
            }
            {
...
test "lower int literal binary op float literal" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
...
test "lower binary op type with int literal" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
    ...
    for (op_strings) |op_string, op_index| {
        for (types) |type_, i| {
            ...
            try expectEqual(int_add.get(components.IrInstructionKind), kinds[i]);
...
test "lower binary op int literal with type" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
...
test "lower binary op type with float literal" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
    ...
    for (op_strings) |op_string, op_index| {
        for (types) |type_, i| {
            const op_table = ops[op_index];
            const kinds = [_]components.IrInstructionKind{ op_table.F64, op_table.F32 };
            var fs = try MockFileSystem.init(&arena);
            _ = try fs.newFile("foo.yeti", try std.fmt.allocPrint(&arena.allocator,
                \\start = function(): {s}
                \\  x: {s} = 10
            ...
            try expectEqual(get_local.get(components.Result).entity, x);
            {
                const float_const = basic_block[3];
                try expectEqual(float_const.get(components.IrInstructionKind), .float_const);
                const result = float_const.get(components.Result).entity;
                try expectEqualStrings(literalOf(result), "32.5");
                try expectEqual(typeOf(result), builtin_types[i]);
            }
            const add = basic_block[4];
            try expectEqual(add.get(components.IrInstructionKind), kinds[i]);
            const result = add.get(components.Result).entity;
            try expectEqual(typeOf(result), builtin_types[i]);
        }
...
test "lower binary op float literal with type" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
...
test "lower int binary op two of same type" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
...
test "lower if then else" {
    ...
    for (types) |type_, i| {
        ...
        try expectEqual(then_block.len, 3);
        {
            ...
            try expectEqual(typeOf(result), builtin_types[i]);
            const set_local = then_block[1];
            try expectEqual(set_local.get(components.IrInstructionKind), .set_local);
            try expectEqual(set_local.get(components.Result).entity, result);
            const get_local = then_block[2];
            try expectEqual(get_local.get(components.IrInstructionKind), .get_local);
            try expectEqual(get_local.get(components.Result).entity, result);
        }
        const else_block = if_.get(components.ElseBlock).entity.get(components.IrInstructions).slice();
        try expectEqual(else_block.len, 3);
        {
...
test "lower if then else conditional is int literal true" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
...
test "lower if then else conditional is int literal is false" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
    ...
    for (types) |type_, i| {
        ...
        try expectEqual(basic_block.len, 4);
        {
            ...
            try expectEqual(int_const.get(components.IrInstructionKind), .int_const);
            const result = int_const.get(components.Result).entity;
            try expectEqualStrings(literalOf(result), "0");
            try expectEqual(typeOf(result), builtins.IntLiteral);
        }
        const int_const = basic_block[1];
        try expectEqual(int_const.get(components.IrInstructionKind), .int_const);
...
test "lower if then else then branch is int literal" {
    var arena = Arena.init(std.heap.page_allocator);
    defer arena.deinit();
    var codebase = try initCodebase(&arena);
    const builtins = codebase.get(components.Builtins);
